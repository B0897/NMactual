...
function calculateKidney() {
    ...
    switch (stage) {
        case 0:
            ...
            break;
        case 1:
            // save left kidney data and launch tool for right kidney
            temp = outlineRegions("leftKidney: ", "right kidney");
            // all roi's information
        ...
        case 5:
            ...
            // automatic save background roi and all image information
            leftBackground = background('left');
            rightBackground = background('right');
            iimage = background('all image');
            // draw activity-time charts
            drawKidneyCurves();
            // calculate gamma aproximation and its integral, draw chart with gamma arpoximation and original curve 
        ...
        case 8:
            ...
            // calculate correlation between curves
            var cov = CalcCor();
        ...
        case 20:
            ...
            rightLung = [];
            rightLungCurve = [];
            rightLungGammaIntegral = [];
            rightLungGammaIntegralLeftFitted = [];
            rightLungGammaIntegralRightFitted = [];
            aorta = [];
            aortaCurve = [];
            aortaGammaIntegral = [];
            aortaGammaIntegralLeftFitted = [];
            aortaGammaIntegralRightFitted = [];
            leftVentricle = [];
            leftVentricleCurve = [];
            leftVentricleGammaIntegral = [];
            leftVentricleGammaIntegralLeftFitted = [];
            leftVentricleGammaIntegralRightFitted = [];
            leftBackground = [];
            rigthBackground = [];
            iimage = [];
...
function outlineRegions(textCurrentOrgan, textNextOrgan) {
    ...
    } else if (stage == 2) { // while saving right kidney roi
...
function drawKidneyCurves() {
    ...
    // create data array containing all regions for iterative data needed to charts creation
    ...
    // create variable containing all data used in charts
    var DataToChart = [];


    for (var regno = 0; regno < 7; regno++) {

        var VectorToChart = [];
        var dataY = [];

        // create normalized activity curve, repeat for all 7 regions
    ...
    // create dataset containing all outlined regions for first chart
    ...
    // prepairing dataset for backgraound chart
    var datasets2 = [];
...
function calculateStatsBackground(imagePixels, imagePixelSpacingColumn, imagePixelSpacingRow, side) {
    ...
    switch (side) {
        case 'right':
            // set coordinates dependent to kidney coordinates
            var roiCoordinates = {
...
var myGammaChart;
function drawGammaFit(organCurve, organLabel) {
    // disply panel for charts
    ...
    // calculate 3 gamma function coefficients and 40s curve
    ...
    // display organ original curve and gamma aproximation chart
    ...
    // create vector with gamma curve for all study time
    ...
    for (var i = 0; i < TimeArray.length; i++) {
        ...
        x = TimeArray[i];
        if (x == 0) { // unexpected infinity result otherwise
            gammaOrganCurveAllStudy[i] = 0;
        } else {
...
function gammaCoefficient(curveArray, gammaTimeArray) {
    ...
    // find maximum activity and calculate from 3 former frames to 3 following 
    ...
    // not startung from 0 to avoid log(0) exception
    var firstMarker = Math.max(maxIndex - 3, 1);
    var secondMarker = Math.min(maxIndex + 3, A);
...
// shift aorta, lung or ventricle gamma integral curve to fit kidney curve
function Pasow(kidney, organIntegral) {
    var end = frameAtTime(40);
    var sumak;
    var previousErrorSum = 1000000000;
    var actualErrorSum = 1000000000;
    ...
    do {
        previousErrorSum = actualErrorSum;
        console.log("sumaErr: "+actualErrorSum)
        ...
        var actualErrorSum = 0; var err = 0;
        for (var i = 0; i < end; i++) {
            err = Math.pow(shiftableIntegral[i] - kidney[i], 2);
            actualErrorSum += err;
        }
        console.log("sumaErr: " + actualErrorSum);
        // repeat until error is decreasing
    } while (actualErrorSum < previousErrorSum);

    // because of loop broke, whe optimum was crossed, stepping backward one iteration
...
// intro to Flow calulation
function calcul() {

    // input patient waight and height if not accesible
    if (!patientHeight) {
        do {
            patientHeight = prompt("Input patient's height:", "[cm]");
        } while (patientHeight < 20 || patientHeight > 300);
    }
    if (!patientWeight) {
        do {
            patientWeight = prompt("Input patient's weight:", "[kg]");
        } while (patientWeight < 1 || patientWeight > 300);
    }
    ...
    //find max value of every organ gamma integral
    for (var i = 0; i < rightLungGammaIntegralLeftFitted.length; i++) {
        maxLungL = Math.max(rightLungGammaIntegralLeftFitted[i], maxLungL);
        maxVentricleL = Math.max(leftVentricleGammaIntegralLeftFitted[i], maxVentricleL);
        maxAortaL = Math.max(aortaGammaIntegralLeftFitted[i], maxAortaL);
        maxLungR = Math.max(rightLungGammaIntegralRightFitted[i], maxLungR);
        maxVentricleR = Math.max(leftVentricleGammaIntegralRightFitted[i], maxVentricleR);
        maxAortaR = Math.max(aortaGammaIntegralRightFitted[i], maxAortaR);
    }
...
//calculate correlation and covariance between kidney curve and organ integral fitted to it
function CalcCor() {
    var start = 0;
    var end = frameAtTime(40);
...
// calculate flow through both kidneys
...
// calculate GFR for both kidneys and total
function calcFiltration() {

    // calculating background and all image curves arrays
    ...
    // from 2 min to 3 min time period
    ...
    for (var i = start; i < end; i++) { // TODO popraw normalizacje na powierzchnie na czas
    ...
    // jj -> number of frames in selected time period
    var jj = end - start + 1;
    int1 = int1 * 60 / jj;
    int2 = int2 * 60 / jj;
